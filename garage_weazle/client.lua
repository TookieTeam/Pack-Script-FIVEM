...
local function location(name)
    ...
    local veh_loc = CreateVehicle(model,-583.445, -930.61, 36.83,270.00, true, false) -- 277.25,-605.21,43.00, 90.00
    TaskWarpPedIntoVehicle(PlayerPedId(), veh_loc, -1)
    SetVehicleNumberPlateText(veh_loc, Config.plate)
    
end

local tookie = false 
RMenu.Add("tookie", "main", RageUI.CreateMenu("~r~Garage Weazle","~b~Description souhaiter"))
        ...
        while tookie do
            ...
            if Config.V4 == true then
                ...
                RageUI.ButtonWithStyle("Ranger la voiture", "Pour ranger une voiture.", {RightLabel = "~r~Ranger"},true, function(Hovered, Active, Selected)
                    if (Selected) then   
                    local weazle,dist4 = ESX.Game.GetClosestVehicle(playerCoords)
                    if dist4 < 4 then
                        DeleteEntity(weazle)
                        RageUI.CloseAll()
                        tookie = false
